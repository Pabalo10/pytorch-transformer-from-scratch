...
class FeedForward(nn.Module):
    ...
    def __init__(self, d_model, expansion, dropout):
        ...
        self.act = nn.GELU()  # activacion de ff - introduce no linealidad
...
class Transformer(nn.Module):
    ...
    def forward(self, idx, causal=True):
        ...
        # TODO arreglado:
        # 1. Creamos la variable 'x' sumando los embeddings
        x = self.tok_emb(idx) + pos
        x = self.drop(x)

        # 2. Pasamos 'x' por cada bloque de atención iterando
